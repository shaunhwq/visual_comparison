...
class ControlButtonsWidget(customtkinter.CTkFrame):
    def __init__(self, icon_manager: IconManager, callbacks, ctk_corner_radius, *args, **kwargs):
        ...
        button_prev = customtkinter.CTkButton(master=frame_00, text="<", command=callbacks["on_prev_file"], width=30, height=25, corner_radius=ctk_corner_radius)
        button_prev.grid(row=0, column=0, padx=(5, 2))
        button_select_specific = customtkinter.CTkButton(master=frame_00, text="File Idx:", command=callbacks["on_specify_index"], width=50, height=25, corner_radius=ctk_corner_radius)
        button_select_specific.grid(row=0, column=1, padx=(0, 2))
        button_next = customtkinter.CTkButton(master=frame_00, text=">", command=callbacks["on_next_file"], width=30, height=25, corner_radius=ctk_corner_radius)
        button_next.grid(row=0, column=2, padx=(0, 5))

        button_search = customtkinter.CTkButton(master=frame_00, text="Filter:", command=callbacks["on_filter_files"], width=50, height=25, corner_radius=ctk_corner_radius)
        button_search.grid(row=0, column=3, padx=5)

        button_prev_method = customtkinter.CTkButton(master=frame_00, text="<", command=callbacks["on_prev_method"], width=30, height=25, corner_radius=ctk_corner_radius)
        button_prev_method.grid(row=0, column=4, padx=(5, 2))
        button_method = customtkinter.CTkButton(master=frame_00, text="Method:", command=callbacks["on_select_methods"], width=50, height=25, corner_radius=ctk_corner_radius)
        button_method.grid(row=0, column=5, padx=(0, 2))
        button_next_method = customtkinter.CTkButton(master=frame_00, text=">", command=callbacks["on_next_method"], width=30, height=25, corner_radius=ctk_corner_radius)
